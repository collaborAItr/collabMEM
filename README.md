...
> presents ACT-R, a cognitive architecture that models human cognition as a set of modular, interacting systems governing memory, perception, and action. The book argues that complex thought can be decomposed into discrete, "atomic" cognitive operations governed by rational principles, supported by computational simulations and empirical psychological data.
...
> Landmark 1763 essay introduces what is now known as Bayes' theorem, providing a mathematical framework for calculating the probability of a cause or hypothesis given observed evidence. It laid the foundation for Bayesian inference — the practice of updating prior beliefs in light of new data — which remains central to statistics, machine learning, and scientific reasoning today.
...
> Paper proposes that semantic memory is organized as a network of interconnected concepts, where activating one concept automatically spreads activation to related concepts based on the strength of their associative links. It extends and revises the earlier Teachable Language Comprehender (TLC) model to better account for empirical findings on semantic priming, typicality effects, and reaction time data in word recognition and categorization tasks.
...
> A mathematical model providing a theoretical framework for computationally identifying records across two files that represent the same person, object, or event, classifying each record pair as a link, non-link, or possible link based on compared characteristics and stipulated error levels. They proved that their probabilistic decision rule is optimal when comparison attributes are conditionally independent, and the framework remains the mathematical foundation for many record linkage applications today.
...
> Foundational neuroscience text introduced what became known as Hebbian learning — the principle that neurons that fire together wire together — proposing that repeated co-activation of neurons strengthens their synaptic connections and underlies learning and memory. Hebb used this framework to bridge neuroscience and psychology, offering a neurological basis for how perception, thought, and behavior are organized.
...
> This W3C specification defines the foundational data model for the Resource Description Framework (RDF), describing how information is represented as triples (subject, predicate, object) to form a graph-based structure for expressing relationships between things on the web. It serves as the core technical reference underpinning the Semantic Web and Linked Data ecosystems.
...
>  Landmark paper that formally developed what is now known as Bayesian inference — the idea that one can reason backwards from observed events to estimate the probability of their underlying causes — laying the mathematical groundwork for inverse probability and statistical inference.
...
> Paper empirically demonstrates that language models exhibit a "U-shaped" performance curve when handling long contexts — performing best when relevant information appears at the very beginning or end of the input, and degrading significantly when that information is located in the middle, even for models explicitly designed for long contexts.
- Quillian, M. R. (1968). Semantic Memory. In M. Minsky (Ed.), *Semantic Information Processing* (pp. 227–270). Cambridge, MA: MIT Press.
> This chapter introduced the concept of semantic memory as a computational model of how human knowledge is organized, proposing a network structure in which concepts are represented as nodes connected by labeled associative links — a framework that became foundational for knowledge representation in both cognitive psychology and early artificial intelligence.
...
> This chapter introduced the Rocchio algorithm, a method for iteratively refining search queries in vector space by moving the query representation closer to relevant documents and farther from irrelevant ones based on user feedback, establishing relevance feedback as a core technique in information retrieval.
...
> Proposes that experiences leave persistent physical traces in biological tissue — which he termed "engrams" — and that these memory traces can be reactivated by stimuli resembling the original experience, a theory that anticipated many modern concepts in neuroscience around memory encoding and retrieval.
...
> Paper provided an early systematic treatment of temporal databases, addressing how to model and query data that changes over time by distinguishing between valid time (when facts hold in the real world) and transaction time (when facts are recorded in the database), laying conceptual groundwork for the field.
...
> Proposes a formal knowledge representation system based on conceptual graphs — a logic-based notation grounded in both linguistics and cognitive science — to model how meaning is structured in human thought and natural language. The book bridges philosophy, psychology, and computer science to lay a theoretical foundation for knowledge engineering and natural language processing in AI systems.
...
> Document serves as an accessible introduction to OWL 2, a formal knowledge representation language built on description logics, explaining how it enables the definition of structured ontologies — with classes, properties, and individuals — to support automated reasoning and semantic interoperability on the web.
